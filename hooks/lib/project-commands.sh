...
is_safe_project_command() {
    ...
    local -a patterns=(
        ...
        # npm/npx
        "^npm (run (test|test:(unit|unit:watch|unit:ui|unit:coverage|integration|integration:coverage|webview|webview:ui|coverage)|build|prod|compile|lint|typecheck|typecheck:(src|unit|integration)|check:undefined|coverage:merge)|test|install)( |$)"
        "^npx (tsc|eslint|vitest|prettier|playwright)( |$)"

        # cargo
        "^cargo (test|check|clippy|build|fmt|doc|run)( |$)"
